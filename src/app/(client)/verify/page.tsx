...
export default function VerifyIdentityPage() {
  ...
  async function loadVerification() {
    ...
    const { data: verif } = await supabase.from('client_verification').select('*').eq('client_id', client.id).single()
    if (verif) {
      setStatus(verif.status)
      setIdRequired(verif.id_required)
    }
  }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!frontFile || !clientId || !idType) return
    setUploading(true)

    const uploadFile = async (file: File, path: string) => {
      const { data } = await supabase.storage.from('client-ids').upload(path, file, { upsert: true })
  ...
  const statusConfig: Record<string, { icon: React.ReactNode; title: string; desc: string; color: string }> = {
    ...
    verified: {
      ...
      desc: 'Your identity has been verified. You can now add trusted contacts and access all features.',
      color: 'bg-green-50',
    },
    rejected: {
      icon: <XCircle size={28} className="text-red-500" />,
      title: 'Verification rejected',
      desc: 'Your verification was not accepted. Please re-submit clear photos of your government-issued ID.',
      color: 'bg-red-50',
    },
  }
  ...
  if (statusInfo && status !== 'unverified') {
    return (
      <div className="max-w-lg mx-auto py-10 px-4 text-center">
        <div className={`${statusInfo.color} w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4`}>
          {statusInfo.icon}
        </div>
        ...
        {status === 'rejected' && (
          <button onClick={() => setStatus('unverified')} className="btn-primary">
    ...
    <div className="max-w-lg mx-auto py-8 px-4">
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-gray-500 leading-relaxed">
          Required to protect your account and enable trusted contact sharing. Your documents are stored securely and never shared.
        </p>
      </div>
