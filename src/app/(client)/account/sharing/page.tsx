...
import { Shield, UserPlus, AlertTriangle, Check, X, Eye, Download, Printer } from 'lucide-react'
...
export default function AccountSharingPage() {
  ...
  async function loadData() {
    ...
    const { data: del } = await supabase.from('account_delegates').select('*').eq('client_id', client.id).order('created_at', { ascending: false })
    setDelegates(del ?? [])
  ...
  async function confirmDelegate() {
    ...
    setSaving(true)
    const { error } = await supabase.from('account_delegates').insert({
      client_id: clientId,
      ...pendingDelegate,
      disclaimer_signed: true,
      disclaimer_signed_at: new Date().toISOString(),
      status: 'pending',
    })
    if (!error) {
    ...
    <div className="max-w-2xl mx-auto py-8 px-4">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Share view-only access to your documents with a family member, guardian, or attorney.
        </p>
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 mt-1 leading-relaxed">
                You must verify your identity before adding trusted contacts. This protects you and your delegates from fraud.
              </p>
              <a href="/client/verify" className="btn-primary text-xs mt-3 inline-block px-4 py-2">
                Verify my identity
              </a>
            </div>
          </div>
            ...
            {delegates.map(d => (
              <div key={d.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {d.status !== 'revoked' && (
                    <button onClick={() => revokeDelegate(d.id)} className="p-1 hover:bg-gray-200 rounded text-gray-400 hover:text-red-500 transition-colors">
                      <X size={13} />
                    </button>
                  )}
                </div>
      ...
      {showForm && (
        <div className="card">
          ...
          <p className="text-xs text-gray-500 mb-5 leading-relaxed bg-gray-50 p-3 rounded-lg">
            This person will be able to view and download your stored documents only. They cannot make changes. Full identity information is required to prevent fraud.
          </p>
          <form onSubmit={handleFormSubmit} className="space-y-3">
            <div>
              <label className="label">Full legal name *</label>
              <input required value={form.delegate_full_name} onChange={e => setForm(f => ({ ...f, delegate_full_name: e.target.value }))} className="input" placeholder="Jane Marie Smith" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="label">Email address *</label>
                <input required type="email" value={form.delegate_email} onChange={e => setForm(f => ({ ...f, delegate_email: e.target.value }))} className="input" placeholder="jane@email.com" />
              </div>
              <div>
                <label className="label">Phone number *</label>
                <input required value={form.delegate_phone} onChange={e => setForm(f => ({ ...f, delegate_phone: e.target.value }))} className="input" placeholder="(904) 555-0100" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="label">Date of birth *</label>
                <input required type="date" value={form.delegate_dob} onChange={e => setForm(f => ({ ...f, delegate_dob: e.target.value }))} className="input" />
              </div>
              <div>
            ...
            <div>
              ...
              <input required value={form.delegate_address} onChange={e => setForm(f => ({ ...f, delegate_address: e.target.value }))} className="input" placeholder="123 Main St, Jacksonville, FL 32201" />
            </div>
          ...
          <div style={{ background: 'white', borderRadius: '12px', width: '100%', maxWidth: '520px', maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div
              onScroll={e => {
                ...
                if (el.scrollHeight - el.scrollTop <= el.clientHeight + 40) setDisclaimerScrolled(true)
              }}
            ...
            {!disclaimerScrolled && (
              <p style={{ textAlign: 'center', fontSize: '11px', color: '#9ca3af', padding: '8px', borderTop: '0.5px solid #f3f4f6' }}>
                Scroll to read the full disclaimer before agreeing
              </p>
            )}
            <div style={{ padding: '14px 20px', borderTop: '0.5px solid #e5e7eb', display: 'flex', gap: '10px' }}>
              ...
              <button
                ...
                disabled={!disclaimerScrolled || saving}
                style={{ flex: 2, padding: '9px', background: disclaimerScrolled ? '#1a6b4a' : '#d1d5db', border: 'none', borderRadius: '8px', color: 'white', fontSize: '13px', cursor: disclaimerScrolled ? 'pointer' : 'not-allowed', fontWeight: 500 }}
              >
